...
import CardHeader from './SimpleModalHeader';
import CardContent from './SimpleModalContent';
import CardFooter from './SimpleModalFooter';
import CardMedia from './SimpleModalMedia';
import CardActions from './SimpleModalActions';

// TODO https://vuetifyjs.com/en/components/dialogs/
// TODO https://github.com/vuetifyjs/vuetify/blob/d31a30020a5e5ad894e12c853bec3576df3dd246/packages/vuetify/src/components/VDialog/VDialog#L154

export enum ESimpleModalVariants {
    ROUNDED = 'rounded',
    OUTLINED = 'outlined',
    FLAT = 'flat',
    PLAIN = 'plain',
...
export enum ESimpleModalMediaAlignment {
    LEFT = 'left',
    RIGHT = 'right'
}

export enum ESimpleModalBorders {
    TOP = 'top',
    BOTTOM = 'bottom',
    LEFT = 'left',
    RIGHT = 'right',
...
const {
    props: variantsProps
} = useComponentVariants<ESimpleModalVariants>({
    enumItem: ESimpleModalVariants
});
...
export default defineComponent({
    ...
    props: {
         ...
         */
        mediaAlignment: {
            type: String as PropType<ESimpleModalMediaAlignment | undefined>,
            required: false,
            validator: (value?: string) => {
                if(!value)
                    return false;
                return Object.values(ESimpleModalMediaAlignment).includes(value as ESimpleModalMediaAlignment);
            }
        },
         ...
         */
        borderPosition: {
            type: Array as PropType<ESimpleModalBorders[]>,
            default: () => [],
            validator: (values?: string[]) => {
                if(!values || values.length === 0)
                    return false;
                return values.every(val => Object.values(ESimpleModalBorders).includes(val as ESimpleModalBorders));
            }
        },
    ...
    setup(props, { slots, attrs }) {
        ...
        const classes = computed(() => [
            ...new Set([
                ...
                props.borderTop || props.borderPosition.includes(ESimpleModalBorders.TOP) ? THEME_CLASS_PREFIX + 'card-border-top-active' : undefined,
                props.borderBottom || props.borderPosition.includes(ESimpleModalBorders.BOTTOM) ? THEME_CLASS_PREFIX + 'card-border-bottom-active' : undefined,
                props.borderRight || props.borderPosition.includes(ESimpleModalBorders.RIGHT) ? THEME_CLASS_PREFIX + 'card-border-right-active' : undefined,
                props.borderLeft || props.borderPosition.includes(ESimpleModalBorders.LEFT) ? THEME_CLASS_PREFIX + 'card-border-left-active' : undefined,
                props.borderFull || props.borderPosition.includes(ESimpleModalBorders.FULL) ? THEME_CLASS_PREFIX + 'card-border-active' : undefined,
